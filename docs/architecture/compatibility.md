...
| 5 | **Widget payload** | `crosscue_widget_v1`, `"version": 1` | App (`HomeWidgetService`) | iOS widget extension (possibly older than the app, or newer after partial update) | Additive only: new optional keys/rows; never repurpose `version` for additive fields | New key name (`_v2`) only for breaking shape changes |
