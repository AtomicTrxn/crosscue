...
| 3 | **Worker D1 schema** | migrations **0001–0007** (0007 = `ops_meta`, the retention heartbeat store, #262) | Worker deploys | Deployed Worker | Migrations must be backward-compatible with the **currently deployed** Worker (additive), because migrate runs before deploy; never edit an applied migration | New numbered SQL file; `migrate → deploy`, staging before prod (DEPLOYMENT.md) |
