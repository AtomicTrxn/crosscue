...
| 2 | H | Worker has zero CI coverage: `ci.yml` runs only Flutter checks; `npm test` / `tsc --noEmit` for `crosscue/backend/challenge_boards` never run on PRs. | Planned (next PR) — path-gated worker-checks job in ci.yml |
| 3 | H | Avatar photos stored as ≤500 KB base64 data-URLs in D1 with no PNG validation, returned inline in every leaderboard row (one 20-member board detail can approach 10 MB). | Planned (worker-hardening PR) — magic-byte validation + size cap first; R2 move / by-reference avatars later |
...
| 5 | M | `previewInvite` returns real board name/member count on the `boardFull`/`playerLimitReached` paths before verifying the invite token — a rotated/expired link still discloses current details. | Planned (worker-hardening PR) — verify token before returning board details |
| 6 | M | Join caps are check-then-act, not transactional (comment at `index.ts` rate-limit helper claims "enforced transactionally"); concurrent joins can briefly exceed 20. | Planned (worker-hardening PR) — correct the comment; bounded overshoot accepted for v1 |
| 7 | M | `requireAuth` writes `last_seen_at` on every authenticated request — one D1 write per API call. | Planned (worker-hardening PR) — throttle the write |
| 8 | M | Date validation is shape-only: `validateDateOnly` accepts `2026-13-99`; `validateIsoDateTime` accepts far-future `completedAt`. | Planned (worker-hardening PR) — real calendar-date check + plausibility window |
| 9 | M | `CHALLENGE_API_ENV=staging|production` silently falls back to sample data unless `CHALLENGE_API_BASE_URL` is also set — a misconfigured release build shows sample boards instead of failing. | Planned (next PR) — fail fast when staging/production lacks a base URL |
