...
| 3 | H | Avatar photos stored as ≤500 KB base64 data-URLs in D1 with no PNG validation, returned inline in every leaderboard row (one 20-member board detail can approach 10 MB). | **Fixed and deployed** — PNG magic-byte validation (#237); R2 by-reference storage + serving (#268); staging/production buckets, bindings, deploys, and remote round-trip smokes verified 2026-07-27 |
