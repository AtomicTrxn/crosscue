...
| 3 | H | Avatar photos stored as ≤500 KB base64 data-URLs in D1 with no PNG validation, returned inline in every leaderboard row (one 20-member board detail can approach 10 MB). | **Fixed and deployed** — PNG magic-byte validation (#237); R2 by-reference storage + serving (#268); staging/production buckets, bindings, deploys, remote round-trip smokes, and the first retention heartbeat verified 2026-07-28 UTC |
...
| 19 | H | Multi-statement board departures and account deletion could leave partial D1 state when a later write failed. | **Fixed (`13375a8`)** — reusable departure statements and one request-level `batch()` cover membership closure, events, board deletion/ownership transfer, results, anonymization, and credential revocation; fault tests require full rollback across multiple boards |
| 20 | M | A result-outbox flush could overwrite a submission enqueued while the flush was in flight. | **Fixed (`e5a0c2b`)** — flush removes only the snapshot it processed and preserves concurrent enqueues; focused race coverage added |
...
| 23 | H | R2 code existed but the account, buckets, bindings, and remote behavior had not been proven end to end. | **Fixed (`ede4d82`, verified 2026-07-28 UTC)** — R2 enabled, both buckets created, staging-first deployment passed, production promoted, token-safe upload/read/delete smokes passed, retention cron heartbeat passed, and cleanup left both buckets empty |
