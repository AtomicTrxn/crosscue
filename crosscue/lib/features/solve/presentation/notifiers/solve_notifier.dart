...
part 'solve_notifier_navigation.dart';
part 'solve_notifier_input.dart';
part 'solve_notifier_check_reveal.dart';
...
class SolveNotifier extends _$SolveNotifier
    with _SolveNavigation, _SolveInput, _SolveCheckReveal {
  ...
  /// breaking the cell autoshrink budget in the painter. Read by the rebus
  /// dialog and enforced in [_SolveInput.inputRebus].
  ...
  /// Safely read the current SolveState from AsyncValue. Concrete impl of the
  /// shared snapshot getter the responsibility mixins depend on.
  @override
  ...
  /// Cancels any pending debounced autosave. Exposed so the terminal reveal
  /// path in [_SolveCheckReveal.revealPuzzle] can stop an in-flight save
  /// without reaching across the mixin boundary for [_saveDebounce].
  @override
  void _cancelPendingSave() => _saveDebounce?.cancel();
