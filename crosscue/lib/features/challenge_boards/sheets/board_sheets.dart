...
Future<T?> showCbSheet<T>(BuildContext context,
                ...
                Flexible(
                    child: Text(title,
                        style: AppTextStyles.titleLarge.copyWith(
                            fontSize: 19, color: AppColors.onSurface1(ctx)))),
                GestureDetector(
                    ...
                    child: Container(
                        ...
                        child: Icon(Icons.close_rounded,
                            size: 18, color: AppColors.onSurface2(ctx)))),
              ]),
              const SizedBox(height: 14),
              builder(ctx),
            ]),
...
Widget _caption(BuildContext c, IconData icon, String text,
    {bool warn = false}) {
  ...
  return Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(
          child: Text(text,
              style:
                  AppTextStyles.caption.copyWith(color: color, height: 1.45))),
    ]),
  );
}
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                  hintText: 'Board name', counterText: ''),
            ),
...
Future<String?> showJoinSheet(BuildContext context) {
  final controller = TextEditingController();
  return showCbSheet<String>(context, title: 'Join a board', builder: (ctx) {
    return StatefulBuilder(builder: (ctx, setState) {
      final value = controller.text.trim();
      return Column(
          mainAxisSize: MainAxisSize.min,
          ...
          children: [
            TextField(
              ...
              autofocus: true,
              onChanged: (_) => setState(() {}),
              decoration: const InputDecoration(hintText: 'Paste invite link'),
            ),
            _caption(ctx, Icons.link_rounded,
...
Future<bool?> showInviteSheet(BuildContext context, InvitePreview preview) {
  return showCbSheet<bool>(context,
...
Future<void> showShareSheet(BuildContext context,
  ...
  return showCbSheet<void>(context,
            ...
            Padding(
              ...
              child: Row(children: [
                Icon(Icons.check_circle_outline_rounded,
                    size: 18, color: AppColors.correct(ctx)),
          ...
          Container(
            ...
            child: Row(children: [
              Icon(Icons.link_rounded,
                  size: 17, color: AppColors.onSurface3(ctx)),
              ...
              GestureDetector(
                ...
                child: Container(
                    ...
                    child: Icon(Icons.copy_rounded,
                        size: 18, color: AppColors.primary(ctx))),
              ),
            ]),
          ),
          _caption(ctx, Icons.schedule_rounded,
    ...
    child: Row(children: [
      Container(
          ...
          child: Icon(Icons.emoji_events_outlined,
              size: 24, color: AppColors.primary(c))),
      ...
      Expanded(
        ...
        Text('${p.playerCount} players · invite-only',
...
Widget _errorBlock(BuildContext c, String title, String body) {
  return Column(children: [
    Container(
        ...
        alignment: Alignment.center,
        decoration: BoxDecoration(
            color: AppColors.incorrect(c).withOpacity(0.14),
            shape: BoxShape.circle),
        child: Icon(Icons.error_outline_rounded,
            size: 24, color: AppColors.incorrect(c))),
    const SizedBox(height: 14),
    Text(title,
